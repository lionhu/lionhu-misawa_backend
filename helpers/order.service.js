...
function get_status_count(options) {
  let url = `${urls.list}status_group_count/`
  return APIServices.post(url,options)
    .then(APIServices.handleResponse)
    .then(response => {
      return response;
    });
...
function updateOrder(info) {
  if (info.order_id) {
    let url = `${urls.list}${info.order_id}/`
    delete info.order_id;
    console.log("updateOrder.service", info)
